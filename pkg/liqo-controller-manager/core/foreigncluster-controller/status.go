...
func (r *ForeignClusterReconciler) handleGatewaysStatus(ctx context.Context,
	...
	if serverCount == 0 {
		...
		fcutils.EnableModuleNetworking(fc)
		allReady, someNotReady := r.checkGatewayDeploymentsReady(ctx, gwServerList.Items)
		switch {
	...
	if clientCount == 0 {
		...
		fcutils.EnableModuleNetworking(fc)
		allReady, someNotReady := r.checkGatewayDeploymentsReady(ctx, gwClientList.Items)
		switch {
		case !allReady && !someNotReady:
			fcutils.EnsureModuleCondition(&fc.Status.Modules.Networking,
...
// checkGatewayDeploymentsReady checks the deployment status for all gateway CRs in the list.
// It returns (allReady, someNotReady) where allReady is true if every deployment has ready replicas,
// and someNotReady is true if at least one deployment is not ready but at least one is.
func (r *ForeignClusterReconciler) checkGatewayDeploymentsReady(ctx context.Context, gateways interface{}) (bool, bool) {
	switch gws := gateways.(type) {
	case []networkingv1beta1.GatewayServer:
		...
		for i := range gws {
			...
			if err != nil {
				klog.Errorf("an error occurred while getting the GatewayServer deployment %q: %s", gw.GetName(), err)
				allReady = false
				continue
			}
			if ready {
		...
		return allReady && anyReady, !allReady && anyReady
	case []networkingv1beta1.GatewayClient:
		...
		for i := range gws {
			...
			ready, err := r.isGatewayDeploymentReady(ctx, gw.GetName(), gw.GetNamespace())
			if err != nil {
				klog.Errorf("an error occurred while getting the GatewayClient deployment %q: %s", gw.GetName(), err)
				allReady = false
				continue
			}
			if ready {
