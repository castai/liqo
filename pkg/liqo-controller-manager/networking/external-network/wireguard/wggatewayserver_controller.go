...
func (r *WgGatewayServerReconciler) Reconcile(ctx context.Context, req ctrl.Request) (res ctrl.Result, err error) {
	...
	replicas := getReplicas(wgServer.Spec.Replicas)
	...
	// Delete obsolete services on scale-down.
	if err = deleteObsoleteServices(ctx, r.Client, wgServer, baseSvcName, replicas); err != nil {
		klog.Errorf("Error while deleting obsolete services: %v", err)
		r.eventRecorder.Event(wgServer, corev1.EventTypeWarning, "ObsoleteServicesDeletionFailed",
			fmt.Sprintf("Failed to delete obsolete services: %s", err))
		return ctrl.Result{}, err
	}
	...
	if err := r.handleEndpointStatus(ctx, wgServer, baseSvcName, replicas); err != nil {
		return ctrl.Result{}, err
	}
...
func (r *WgGatewayServerReconciler) ensureService(ctx context.Context, wgServer *networkingv1beta1.WgGatewayServer,
	...
	op, err := resource.CreateOrUpdate(ctx, r.Client, &svc, func() error {
		return r.mutateFnWgServerService(&svc, wgServer, replica)
	})
	if err != nil {
	...
	// Ensure the Service is owned by the per-replica Deployment, so scaling down the
	// replica automatically garbage-collects the Service.
	if err := r.setServiceDeploymentOwner(ctx, wgServer, &svc, replica); err != nil {
		return nil, err
	}
...
// setServiceDeploymentOwner sets the per-replica Deployment as owner of the Service.
func (r *WgGatewayServerReconciler) setServiceDeploymentOwner(ctx context.Context, wgServer *networkingv1beta1.WgGatewayServer,
	svc *corev1.Service, replica int32) error {
	...
	if err := r.Get(ctx, depNsName, &deployment); err != nil {
		return fmt.Errorf("getting owner deployment %q for service %q: %w", depNsName, client.ObjectKeyFromObject(svc), err)
	}
	if err := controllerutil.SetOwnerReference(&deployment, svc, r.Scheme); err != nil {
		return fmt.Errorf("setting deployment owner reference on service %q: %w", svc.Name, err)
	}
...
func (r *WgGatewayServerReconciler) handleEndpointStatus(ctx context.Context, wgServer *networkingv1beta1.WgGatewayServer,
	baseSvcName string, replicas int32) error {
...
func (r *WgGatewayServerReconciler) forgeEndpointStatusNodePort(ctx context.Context, service *corev1.Service) (*networkingv1beta1.EndpointStatus, error) {
	...
	node := &corev1.Node{}
	err := r.Get(ctx, types.NamespacedName{Name: activePod.Spec.NodeName}, node)
