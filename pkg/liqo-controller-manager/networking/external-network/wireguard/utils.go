...
func deleteObsoleteDeployments(ctx context.Context, cl client.Client, owner metav1.Object, baseName string, replicas int32) error {
	...
	for i := range deployments.Items {
		...
		var idx int
		if _, err := fmt.Sscanf(dep.Name, baseName+"-%d", &idx); err != nil {
			continue
		}
		if int32(idx) >= replicas {
			if err := cl.Delete(ctx, dep); err != nil && !kerrors.IsNotFound(err) {
...
// deleteObsoleteServices removes services with an index greater than or equal to replicas,
// and also deletes legacy services created before the gw- prefix was introduced.
func deleteObsoleteServices(ctx context.Context, cl client.Client, owner metav1.Object, baseName string, replicas int32) error {
	var services corev1.ServiceList
	if err := cl.List(ctx, &services, client.InNamespace(owner.GetNamespace())); err != nil {
	...
	for i := range services.Items {
		...
		// Delete legacy services created before per-replica naming was introduced.
		// Old single-replica services were named gw-<owner.Name> (i.e. baseName),
		// while new per-replica services are named gw-<owner.Name>-<idx>.
		if svc.Name == baseName {
			if err := cl.Delete(ctx, svc); err != nil && !kerrors.IsNotFound(err) {
				return fmt.Errorf("deleting legacy service %q: %w", svc.Name, err)
			}
			klog.Infof("Deleted legacy service %q", svc.Name)
			continue
		}

		var idx int
		if _, err := fmt.Sscanf(svc.Name, baseName+"-%d", &idx); err != nil {
			continue
		}
		if int32(idx) >= replicas {
			if err := cl.Delete(ctx, svc); err != nil && !kerrors.IsNotFound(err) {
				return fmt.Errorf("deleting service %q: %w", svc.Name, err)
			}
			klog.Infof("Deleted obsolete service %q", svc.Name)
		}
	}
...
// setControllerReference sets the given object as owner, handling errors.
