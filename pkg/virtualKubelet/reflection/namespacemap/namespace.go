...
// An error (e.g., listing failure, or no NamespaceMap known yet, which is always transient during the
// virtual kubelet lifetime) marks the state as uncertain: callers are expected to retry instead of
// taking irreversible actions on potentially incomplete information. Multiple NamespaceMaps (possible
// during delete+recreate cycles) are checked, rather than requiring uniqueness.
